...
from auth.authentications import get_password_hash
from models.db import SessionLocal
from models.models import Municipality, User, UserRole, State
...
def load_states_and_municipalities():
    db = SessionLocal()  # Create a new session
    try:

        statesExists = db.query(State).all()
        if statesExists:
            return

        # Dictionary to track already added states
        states = {}
        ...
        for item in data:
            ...
            if c_digo_dane_del_departamento not in states:
                state = State(
                    region=item["region"],
                ...
                db.add(state)
                db.flush()  # Flush to assign an ID to the state before adding municipalities
                states[c_digo_dane_del_departamento] = state
            ...
            municipality = Municipality(
                ...
                name=item["municipio"],
                state_id=states[c_digo_dane_del_departamento].id
            )
            db.add(municipality)
        ...
        db.rollback()
        logger.error(f"An error occurred while loading states and municipalities: {e}")
    finally:
        db.close()
...
def load_users():
    ...
    try:
        ...
        myuser = User(
            ...
            password=get_password_hash("password123"),  # In real applications, use hashed passwords
            rol=UserRole.ADMIN,
            municipality_id=2,
            address="123 Main St",
        ...
        db.rollback()
        logger.error(f"An error occurred while loading municipalities: {e}")
    finally:
        db.close()
